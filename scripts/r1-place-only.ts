...
console.log("[r1] place-only (skip shield; reuse existing private tUSDC)");
...
// SAVE FIRST (before reveal) so the placement is committed the instant the order
// is on-chain — a retry loop must NEVER place a second order into the same epoch
// (that orphans order_acc: order_count grows but reveals don't match). The reveal
// is a local HTTP POST below; if it ever fails it is re-revealable, but placeOrder
// is NOT idempotent, so commit on placeOrder success.
s.submittedOrder = {
...
console.log("[r1] saved submittedOrder to sub9-e2e-state.json — epoch " + res.epoch);
...
console.log("[r1] revealed to local agg ok=" + ok + " — epoch " + res.epoch + " should CLEAR");
await cl.stop?.();
